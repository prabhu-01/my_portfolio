...
export default function HeroVisual() {
  ...
  /* Raw mouse offset from container center (pixels) */
  const rawX = useMotionValue(0)
  const rawY = useMotionValue(0)

  /* Map to rotation degrees */
  const rotateX = useSpring(
    useTransform(rawY, [-250, 250], [7, -7]),
    { stiffness: 55, damping: 18 }
  )
  const rotateY = useSpring(
    useTransform(rawX, [-250, 250], [-7, 7]),
    { stiffness: 55, damping: 18 }
  )

  /* Subtle ambient X/Y translation (smaller movement for depth) */
  const px = useSpring(useTransform(rawX, [-250, 250], [-6, 6]),  { stiffness: 55, damping: 18 })
  const py = useSpring(useTransform(rawY, [-250, 250], [-6, 6]),  { stiffness: 55, damping: 18 })
  ...
  return (
    /* Perspective wrapper — must NOT be the rotated element */
    <div
      style={{ perspective: '1100px', perspectiveOrigin: '50% 50%', height: 570 }}
      onMouseMove={onMouseMove}
      onMouseLeave={onLeave}
    >
      {/* This element rotates in 3D */}
      <motion.div
        ref={containerRef}
      ...
      >
        ...
        <div className="float-a absolute" style={{ top: 0, left: 8, width: 252 }}>
          ...
          >
            <div
              className="rounded-2xl p-5 glass-premium cursor-pointer"
              style={{
                ...CARD,
                border: '1px solid rgba(167,139,250,0.22)',
                boxShadow: '0 0 50px rgba(124,58,237,0.12), inset 0 1px 0 rgba(255,255,255,0.08)',
              }}
            >
        ...
        <div className="float-c absolute" style={{ top: 190, left: 0, width: 290 }}>
          <motion.div
            initial={{ opacity: 0, y: 24, scale: 0.95 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            ...
            >
              ...
              <div className="flex flex-wrap gap-1.5">
                {STACK.map((s, i) => (
                  <motion.span
                    key={s.label}
                    className="text-[11px] font-medium px-2.5 py-1 rounded-lg transition-all duration-200 cursor-default"
                    ...
                    whileHover={{ scale: 1.1, background: `${s.color}28` }}
                  >
                    {s.label}
                  </motion.span>
                ))}
        ...
        <div className="float-d absolute" style={{ top: 210, right: 4, width: 188 }}>
          <motion.div
            initial={{ opacity: 0, y: 24, scale: 0.95 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
          ...
          >
            <div
              ...
              style={{
                ...
                boxShadow: '0 0 40px rgba(34,211,238,0.08), inset 0 1px 0 rgba(255,255,255,0.06)',
              }}
            >
              <div className="flex items-center gap-2 mb-3">
          ...
          >
            <div
              ...
              style={{
                ...
                boxShadow: '0 0 40px rgba(34,211,238,0.09), inset 0 1px 0 rgba(255,255,255,0.07)',
              }}
            >
        ...
        {/* ── Card 6: Currently reading ── float-b offset */}
          ...
          >
            <div
              ...
              style={{
                ...
                boxShadow: '0 0 30px rgba(251,191,36,0.07), inset 0 1px 0 rgba(255,255,255,0.06)',
              }}
            >
